...
scene::IAnimatedMeshSceneNode *rabbitNode = NULL;
...
void createRabbit()
{
    ...
    rabbitNode->setAnimationSpeed(12000);
    rabbitRadius = rabbitNode->getBoundingBox().getExtent().Y/2.0;
    yVel = 0.0;
    jumpVel = jumpVelMax;
...
{
    core::vector3df pos = rabbitNode->getAbsolutePosition();
    ...
    if (!rabbitFallen)
    {
        ...
        {
            rabbitNode->setRotation(core::vector3df(45, 0,0));
        }
        rabbitNode->setLoopMode(yVel >= 0);
        //nodePS->setVisible(yVel < 0);
    }
    else
    {
        //smgr->getActiveCamera()->setPosition(pos+core::vector3df(0,20.0-pos.Y,-30.0));
        rabbitNode->setRotation(rabbitNode->getRotation()+core::vector3df(2,2,2)*60.0*dt);
    }
    ...
    rabbitNode->updateAbsolutePosition(); // fix1
}
...
{
    core::vector2df pos1 = core::vector2df(rabbitNode->getAbsolutePosition().Y, rabbitNode->getAbsolutePosition().Z);
...
bool rabbitDied()
{
    f32 diff = rabbitNode->getAbsolutePosition().Y-getLowestPlatformPoint().Y;
